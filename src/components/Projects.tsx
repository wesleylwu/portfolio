...
const Projects = () => {
  return (
    <>
      <p className="text-glow-red text-red-neon text-center p-8 text-5xl">Projects</p>
      <div className="mx-auto grid max-w-7xl grid-cols-2 gap-8">
        {projects.map(({ website, alt, name, description, icons, github, websiteLink }) => {
          const githubIndex = icons.indexOf(SiGithub);
          const leftIcons = githubIndex !== -1 ? icons.filter((_, i) => i !== githubIndex) : icons;

          return (
            <div key={name} className="flex flex-col items-center">
              <Link href={websiteLink} target="_blank" className="w-2/3 rounded-t-3xl overflow-hidden">
